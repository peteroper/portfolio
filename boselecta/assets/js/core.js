...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (heading && description && button) {
      ...
      description.style.opacity = '24%';
      button.style.opacity = '24%';
  
      // Add scroll event listener
      window.addEventListener('scroll', function () {
        if (window.scrollY > scrollTrigger) {
          heading.style.opacity = '24%'; // H1 to 24%
          ...
          description.style.opacity = '24%'; // Hide when not scrolled enough
          button.style.opacity = '24%'; // Hide button initially
        }
      });
    }
  });
